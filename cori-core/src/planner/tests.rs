...
mod tests {
    ...
    fn test_display_symbols() {
        let (mut list, _f) = temp_list();
        list.write(vec![
            Task { id: "1".into(), description: "a".into(), state: TaskState::Pending },
            Task { id: "2".into(), description: "b".into(), state: TaskState::InProgress },
            Task { id: "3".into(), description: "c".into(), state: TaskState::Completed },
        ])
        .unwrap();
